...
export class DeepJSONConnector {
  constructor(config) {
    ...
    // Sliding session: every authenticated response carries the token for the
    // next request in X-Renewed-Token. Picking it up centrally keeps every
    // endpoint (and the socket handshake) on the freshest token.
  ...
  async uploadFile(key, file, options = {}) {
    let form;
    if (this.isNode) {
      const fs = (await import('fs')).default;   // ← lazy, browser-safe
      const stream = fs.createReadStream(file);
      const stats = fs.statSync(file);
      form = new FormData();
      form.append('file', stream, { filename: file.name, size: stats.size });
    } else {
      form = new FormData();
